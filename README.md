...
| A 3x3 square board must be accessible from the screen once the application is run (either directly or through a menu) | A 3x3 square board is visible when you run the app, then click the "play" button on the menu screen that appears. This is a `Board` object created in the `GameScreen` class in `tic/screens/GameScreen` |
...
| Display the state of the game on a square board that scales with window size. The board must remain square at all times, no matter the window’s aspect ratio | This should be visible by resizing the window.  |
