...
| A 3x3 unit board must be accessible from the screen once the application is run (either directly or through a menu) | A 3x3 unit board is visible when you run the app, then click the "play" button on the menu screen that appears. This is a `Board` object created in the `GameScreen` class in `tic/screens/GameScreen` |
